...
import { GOLD, LIGHT, page, center, goldBtn, card, AudioButton, FeedbackScreen, postToAirtable, speakText, detectPlatform } from "./shared.jsx";
...
function drawPin(ctx, x, y, r) {
  ctx.save();
  ctx.beginPath(); ctx.ellipse(x, y + r * 0.15, r * 1.05, r * 1.25, 0, 0, Math.PI * 2);
  ctx.fillStyle = "#f8fafc"; ctx.fill();
  ctx.lineWidth = Math.max(1, r * 0.14); ctx.strokeStyle = "#94a3b8"; ctx.stroke();
  ctx.beginPath(); ctx.ellipse(x, y - r * 0.35, r * 0.55, r * 0.32, 0, 0, Math.PI * 2);
  ctx.fillStyle = "#ef4444"; ctx.fill();
  ctx.restore();
}
function drawBall(ctx, x, y, r) {
  ctx.save();
  ctx.shadowColor = "#000"; ctx.shadowBlur = r * 0.6;
  ctx.beginPath(); ctx.arc(x, y, r, 0, Math.PI * 2);
  const grad = ctx.createRadialGradient(x - r * 0.3, y - r * 0.3, r * 0.1, x, y, r);
  grad.addColorStop(0, "#7dd3fc"); grad.addColorStop(1, "#1e3a5f");
  ctx.fillStyle = grad; ctx.fill();
  ctx.shadowBlur = 0;
  ctx.lineWidth = Math.max(1, r * 0.1); ctx.strokeStyle = "#0b0f14"; ctx.stroke();
  ctx.restore();
}
  ...
  return (
    <div style={{ display: "flex", overflowX: "auto", gap: 3, marginBottom: 16, paddingBottom: 4 }}>
      {frames.map((fr, i) => {
        ...
        return (
          <div key={i} style={{ minWidth: i === 9 ? 66 : 48, background: active ? "#1e3a5f" : "#1e293b", border: `2px solid ${active ? "#7dd3fc" : "#334155"}`, borderRadius: 10, padding: "6px 4px", flexShrink: 0 }}>
            <p style={{ color: LIGHT, fontSize: 11, fontWeight: "bold", textAlign: "center", margin: "0 0 4px" }}>{i + 1}</p>
            <div style={{ display: "flex", justifyContent: "center", gap: 3, marginBottom: 4 }}>
              {(i === 9 ? [0, 1, 2] : [0, 1]).map(ri => {
                ...
                return <span key={ri} style={{ color: sym === "X" || sym === "/" ? GOLD : "#e2e8f0", fontSize: 14, fontWeight: "bold", minWidth: 12, textAlign: "center" }}>{sym}</span>;
              })}
            </div>
            <p style={{ color: GOLD, fontSize: 15, fontWeight: "bold", textAlign: "center", margin: 0 }}>{sc ? sc.cumulative : "—"}</p>
          </div>
        );
      })}
    </div>
...
export default function BowlingGame({ onBack }) {
  ...
  function draw() {
    ...
    drawPoly(gutterLCorners, "#111827"); drawPoly(gutterRCorners, "#111827");
    drawPoly(laneCorners, "#c8956d");
    ctx.save();
    const grad = ctx.createLinearGradient(laneCorners[0].x, laneCorners[0].y, laneCorners[3].x, laneCorners[3].y);
    grad.addColorStop(0, "#a9744f"); grad.addColorStop(1, "#d8ab7f");
    drawPoly(laneCorners, grad);
    ctx.restore();
    ctx.lineWidth = Math.max(2, w * 0.005); ctx.strokeStyle = "#3f2c1d";
    ...
    if (aimStateRef.current && !animatingRef.current) {
      ...
      const endX = ball.x + dirX * t, endY = ball.y + dirY * t;
      const p1 = tableToScreen(ball.x, ball.y), p2 = tableToScreen(endX, endY);
      ctx.save();
      ctx.setLineDash([w * 0.016, w * 0.012]);
      ctx.lineWidth = Math.max(2, w * 0.006);
      ctx.strokeStyle = "#ffffff"; ctx.shadowColor = GOLD; ctx.shadowBlur = 10;
  ...
  if (screen === "home") return (
    <div style={center}><div style={{ maxWidth: 640, width: "100%", textAlign: "center" }}>
      ...
      <h1 style={{ fontSize: 36, fontWeight: "bold", color: GOLD, marginBottom: 10 }}>Bowling</h1>
      <p style={{ fontSize: 22, color: LIGHT, fontWeight: "bold", marginBottom: 20, lineHeight: 1.5 }}>Drag to aim, pull back to power your throw, and knock down all ten pins — a classic game of aim and timing.</p>
      <div style={{ ...card, textAlign: "left", marginBottom: 20 }}>
        <p style={{ color: "#7dd3fc", fontSize: 17, fontWeight: "bold", marginBottom: 14 }}>HOW TO PLAY</p>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {[["👆", "Drag anywhere on the lane, pull back from the ball"], ["🎯", "Release to roll — further pull-back means more power"], ["🎳", "Two rolls per frame, unless you strike on the first"], ["🏆", "Ten frames make a full game — strikes and spares earn bonus points"]].map(([icon, text], i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 14, background: "#0f172a", borderRadius: 10, padding: "12px 16px" }}><span style={{ fontSize: 28 }}>{icon}</span><span style={{ color: "#e2e8f0", fontSize: 19, fontWeight: "bold" }}>{text}</span></div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap", marginBottom: 20 }}>
        <button onClick={() => { resetGame(); setScreen("game"); }} style={goldBtn}>Start Game →</button>
      ...
      <div style={{ maxWidth: 940, width: "100%" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          ...
          <div style={{ textAlign: "center" }}>
            <span style={{ color: GOLD, fontSize: 17, fontWeight: "bold" }}>🎳 Frame {Math.min(currentFrame + 1, 10)} of 10</span>
            <span style={{ color: LIGHT, fontSize: 16, fontWeight: "bold", display: "block" }}>Score: {(() => { const done = scores.filter(Boolean); return done.length ? done[done.length - 1].cumulative : 0; })()}</span>
          </div>
          <button onClick={resetGame} style={{ background: "#1e293b", color: LIGHT, border: "1px solid #334155", borderRadius: 10, padding: "10px 18px", fontSize: 15, fontWeight: "bold", cursor: "pointer" }}>🔄 Reset</button>
        </div>
        <Scoreboard frames={frames} scores={scores} currentFrame={currentFrame} />
        <div style={{ position: "relative", width: "100%", maxWidth: 620, aspectRatio: "3/4", margin: "0 auto 16px", borderRadius: 20, overflow: "hidden", touchAction: "none", background: "#000", border: "2px solid #334155" }}>
